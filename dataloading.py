...
class DistributedPaddedIterableDataset8(IterableDataset):
    """An IterableDataset that handles distributed padded data loading with masking.
    Pads sequences to the nearest multiple of 8 instead of a fixed sequence length.
    """
    ...
    def _pad_to_multiple_of_8(self, length: int) -> int:
        """Calculate the nearest multiple of 8 that is >= length."""
        return ((length + 7) // 8) * 8
    ...
    def __iter__(self):
        ...
        while epoch < self.max_epochs:
            ...
            for i in range(len(eos_positions)):
                ...
                # Handle samples that exceed max sequence length
                if len(sample) > self.max_seq_len:
                    # Split large samples into multiple batches
                    for j in range(0, len(sample), self.max_seq_len):
                        chunk = sample[j:j+self.max_seq_len]
                        # Pad chunk to nearest multiple of 8
                        padded_len = self._pad_to_multiple_of_8(len(chunk))
                        if len(chunk) < padded_len:
                            padding = torch.full((padded_len - len(chunk),), self.pad_token_id, dtype=torch.uint8)
                            chunk = torch.cat([chunk, padding])
                ...
                # We yield when we have a reasonable amount of tokens to pad to multiple of 8
                if curr_batch_len >= 512:  # Arbitrary threshold to create reasonably sized batches
                    batch = torch.cat(batch_tokens)
                    # Pad to nearest multiple of 8
                    padded_len = self._pad_to_multiple_of_8(len(batch))
                    if len(batch) < padded_len:
                        padding = torch.full((padded_len - len(batch),), self.pad_token_id, dtype=torch.uint8)
